...
UINT8       Set_OK=0;
UINT8		EOT_Flag=0;
UINT8	    f_FirstComm=0xaa;//init. value is "0xaa" -> "0x55"
// Input_Voltage
...
void Serial_Process(void)
{
    ...
    if(f_SData && f_EData)
    {
        Response_Serial();
        //---------------------------
//        if(f_FirstComm == 0xaa)//init.value is 0xaa//
//        {
//            f_FirstComm=0x55; //first get data from Rect.Display//
//            //Set_Value();
//            Set_OK = 1;
//        }
        //-----------------------------
    ...
    {    
        ...
        if(tmr2Count3 >= RS232_Period)				//elapse time about 500ms~750ms
        {
            ...
            else if(Set_OK == 0)
            {
                ...
                tmpvalue = Input_Spare1*10;             //Spare1
                Send_Serial(tmpvalue,4,'C');

                tmpvalue = Input_Spare2*10;             //Spare2
                Send_Serial(tmpvalue,5,'C');

                tmpvalue = Input_Spare3*10;             //Battery Current FB
